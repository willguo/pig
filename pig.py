...
def take_turn(plan, dice=make_fair_die(), who='Someone', comments=False):
    ...
    """
    turn_total, score_for_turn = 0, 0  # Points scored in the whole turn
    "*** YOUR CODE HERE ***"
    turn_over = False
    ...
    while turn_over != True:
        ...
        (score_for_turn, turn_total, turn_over) = action(turn_total, temp_dice)

        if comments:
            commentate (action, temp_dice, score_for_turn, turn_total, turn_over, who)
...
def take_turn_test():
    ...
    #assert take_turn(plan, make_test_die(6, 2, 5, 5)) == 13
    #assert take_turn(plan, make_test_die(6, 3, 1, 5, 4, 6)) == 1
...
def averaged(fn, num_samples=1000):
    ...
    def average_and_return(*args):
        ...
        while index < num_samples:
            total_value = total_value + fn(*args)
            index += 1
...
def eval_strategy_range(make_strategy, lower_bound, upper_bound):
    ...
    while value <= upper_bound:
        ...
        value += 1
    print('The best number to use against the baseline is:')
    return best_value
...
def run_strategy_experiments():
    ...
    #print (eval_strategy_range(make_die_specific_strategy, 5, 15))
    #print (eval_strategy_range(make_pride_strategy, 0, 10))
    #print (eval_strategy_range(final_strategy, 0, 100)) <--fix up
...
def make_die_specific_strategy(four_side_goal, six_side_goal=20):
    ...
    four_side_goal -- the roll-until goal whenever the turn uses a 4-sided die, best = 7
    six_side_goal -- the roll-until goal whenever the turn uses a 6-sided die, best = 22

    """
    "*** YOUR CODE HERE ***"
    def strategy_die_specific (score1, score2):
...
def make_pride_strategy(margin, turn_goal=20):
    ...
    """
    "*** YOUR CODE HERE ***"
    def strategy_pride(score1, score2):

        goal = turn_goal
        
        if score1 <= (score2 + margin):
            goal = ((score2 + margin) - score1)
        
        return make_roll_until_plan(goal)
    
    return strategy_pride
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    This strategy utilizes a few ideas/concepts to form a plan:
    -the idea of holding as soon as a player hits 100 (rather than continuing to roll) is beneficial--it's a win.
    -the idea of holding whenever the sum of your score and your opponent's score is a multiple of 7 (giving them a 4-sided die)
    -the idea of rolling a 4-sided die for a turn goal of 7, rather than 22, is better (22 is much harder to get with a 4-sided die))
    -roll until 22 in all other cases

    """
    "*** YOUR CODE HERE ***"
    
# FINAL STRATEGY HERE
    def final_plan(turn_total):

        if (score > 100):
            return hold

        if ((turn_total + score + opponent_score) % 7 == 0 and turn_total > 8):
            return hold
